...
package UMTS::GUI::Phonebook_Sel;
...
sub new
{
  ...
  my $lbl_pbook = new Gtk2::Label( 'Terminal phonebooks' );
  $termbar->pack_start( $lbl_pbook, FALSE, FALSE, 0 );

  $self->{combo} = new Gtk2::Combo();
  $self->{combo}->set_popdown_strings(@pbooks);
  $termbar->pack_start( $self->{combo}, FALSE, FALSE, 0 );
